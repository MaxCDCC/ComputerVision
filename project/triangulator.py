...
def triangulate_point(observations: Sequence[Tuple[np.ndarray, np.ndarray]]) -> Optional[np.ndarray]:
    # solve a linear system to find a 3D point from multiple 2D projections
...
def compute_reprojection_error(point_3d: np.ndarray, camera: CameraModel, uv: np.ndarray) -> float:
    # project the 3D point into a view and compute pixel distance
    point_3d = point_3d.reshape(1, 1, 3)
    projected, _ = cv2.projectPoints(point_3d, camera.rvec, camera.t, camera.K, camera.dist)
...
def process_dataset(annotation_path: str, calib_root: str) -> dict:
    # load annotations and calibrations, then run triangulation
    ...
    for frame_idx, players in sorted(grouped.items()):
        ...
        for category_id, view_list in sorted(players.items()):
            # for each player, collect their 2D views
            player_result = {'views': {}, 'triangulated_points': []}
            ...
            for kp_idx in range(n_points):
                ...
                mean_err = float(np.mean(point_error)) if point_error else None
                valid_points.append({
